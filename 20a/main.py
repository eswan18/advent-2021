...
class Image:
    ...
    def __init__(self, pts: list[list[pt]]):
        self.pts = pts
        self.n_rows = len(pts)
        self.n_cols = len(pts[0])

    @classmethod
    def from_string(cls, s) -> Image:
    ...
    def __getitem__(self, key: tuple[int, int]) -> pt:
        if isinstance(key, tuple):
            ...
            if 0 <= x < self.n_cols and 0 <= y < self.n_rows:
                ...
                # Indexing to a location off the board just gets you a dark square.
                return '.'
        else:
            raise TypeError
    ...
    def enhance(self, key: str) -> Image:
        ...
        for y in range(-1, self.n_rows+1):
            for x in range(-1, self.n_cols+1):
                ...
                new_grid += key[loc]
            new_grid += '\n'
        return self.__class__.from_string(new_grid.strip())
