...
class Scanner:
    ...
    def from_text(cls, text) -> 'Scanner':
        ...
        return cls(number=number, beacons=beacons, absorbed=[Coord(0, 0, 0)])
    ...
    def reorient(self, other: 'Scanner') -> 'Scanner':
        ...
        # out rotation.
        c2_vector = facing(cls(number=0, beacons=[c2_vector], absorbed=[])).beacons[0]
        x2, y2, z2 = c2_vector
        if z2 != z1:
            raise RuntimeError
        ...
        c2_vector = rotate(cls(number=0, beacons=[c2_vector], absorbed=[])).beacons[0]
    ...
    def __add__(self, other: 'Scanner') -> 'Scanner':
        ...
        other_absorbed = [
            Coord(c.x + offset[0], c.y + offset[1], c.z + offset[2])
            for c in other.absorbed
        ]
        return self.__class__(
            number=self.number,
            beacons=new_beacons,
            location=self.location,
            absorbed=self.absorbed + other_absorbed,
        )
